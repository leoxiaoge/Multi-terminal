...
|名称| 类型| 默认值| 是否必须| 说明|
| :----- | :-----: | :-----: | :-----: | :----- |
|sourcedir|string|| 是|项目所在路径 |
|appname|string|| 是|打包的项目名字 |
|--platform=<platform>|string|| 是|确定了你要构建哪个平台的应用（Windows、Mac 还是 Linux）  |
|--arch=<arch>|string|| 是|决定了使用 x86 还是 x64 还是两个架构都用  |
|[optional flags...]|string|| 是|可选选项 |
